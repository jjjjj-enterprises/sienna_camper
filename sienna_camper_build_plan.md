...
**The walls are 13.75" tall, not 14.5" (corrected Sept 2026).** 14.5" is `leg_height 17 − 1.5 − 1.0` — the bay as it stood *before* the deck recess dropped Panels A/B's legs by 3/4". The bay is now `leg_height_ab 16.25 − 1.5 − 1.0 = 13.75"`, and a 14.5" box does not go under the deck at all. The wall height simply never followed the legs down. Measure your own frame's bay before cutting — top of the bottom rail up to the underside of the top rail — since it is set by the legs' cut length and is *not* affected by how far the leveling feet are wound out.

**Why 3/4" for the walls and not 1/2".** Not preference — capacity. The 1/2" sheet's leftover holds the front wall and the tray and then two of the four walls, and stops: every remaining offcut is under 27.5" in its short direction, so each yields exactly one 13.75" strip and no more. The only leftovers that do are 3/4". Mixing (two 1/2" walls, two 3/4") would break the box's 20"/22" arithmetic and its biscuit sizing, so the box is one thickness throughout.
...
| **Battery drawer side wall** | 2 | **22" x 13.75"** — full depth, and the slide pair screws to these two. One out of the 47" x 19" strip, one out of the anchor board's **gap A** (16.85" x 27", so it comes out rotated). 3/4" (was 3/8"): glue + **2× R3** per corner |
| **Battery drawer front/back wall** | 2 | **18.5" x 13.75"** — they fit BETWEEN the side walls (owner, Sept 2026), so cut them at 20" with everything else and trim **1.5" (= 2 × 3/4")** off each once the sides exist. One out of the same 47" x 19" strip beside the side wall (22 + 20 = 42 of the 47), one out of the anchor board's **gap B** (20.5" x 27", also rotated). 3/4", 2× R3 per corner |
...
| Battery drawer bottom (Panel A right) | 1 | **19.25" x 21.25"** (1/2" — the base under the 48 lb stack. **20 x 22 is the box's OUTER footprint**, which is the size for a bottom sitting *under* the walls; this one is housed in a 3/8" glued rabbet, and that takes 3/4" off each dimension. Cut it last, off the glued-up box, not to the number) |
...
**Both sheets are cut, and they broke down as drawn** — no dimension, no part and no offcut in the tables above changed. What these add is the *method*, and in particular the one thing this build has that no rectangle needs: the anchor board's comb has **interior corners**, and how you get into them is the whole job. The division of labour turned out to be two saws — **table saw for every long cut, jigsaw for every interior cut** — with a drill bit standing between them.

<figure class="photo-figure">
<img src="photos/ply-01-cut-plan-on-sheet.jpg" alt="A large plywood panel on the saw table with the printed cut plan, a four-foot level, a tape measure and a pencil laid on it">
<figcaption><b>Mark off the printed cut plan, on the sheet, before any blade runs.</b> The sheet layout from Section 3 is printed and sitting on the work — not read off a phone and not held in memory. The straightedge here is a <b>4&nbsp;ft level</b>, which is the longest true edge most shops own and is what a marked line should be struck against; the face grain direction gets confirmed at this stage too, because for the anchor board it is specified (<b>grain along the 33" direction</b>) and it is unrecoverable once the piece is cut. Note what has already happened before this photo: the store broke the sheets into rectangles (Section 3), so what is on the table is a manageable panel rather than a full 4×8.</figcaption>
</figure>

<figure class="photo-figure">
<img src="photos/ply-03-table-saw-setup.jpg" alt="A Ryobi BT3000 table saw set up in the garage with its rip fence and rails">
<figcaption><b>Every long cut went on the table saw.</b> Section 3's cut plan offers a track saw or a circular saw on a straightedge; the saw that was in the shop was a table saw, and for a sheet already broken into rectangles it is the better of the three — a fence gives a repeatable width, where a straightedge has to be re-clamped and re-measured for every cut. <b>What a table saw cannot do is the part that makes this board a comb:</b> the two gaps have to <i>stop</i> at the bridge line, and the cut that closes each gap runs perpendicular to the rip. That cut is the jigsaw's, below.</figcaption>
</figure>

<figure class="photo-figure">
<img src="photos/ply-04-jigsaw-reverse-blade.jpg" alt="A SKIL jigsaw beside a pack of Bosch T101BR wood blades on a plywood surface">
<figcaption><b>Put a reverse-tooth blade in the jigsaw — this is the detail worth stealing.</b> The pack is <b>Bosch T101BR</b>, 10 TPI: the <b>R is for reverse</b>, meaning the teeth point <i>down</i> instead of up. An ordinary jigsaw blade cuts on the upstroke and therefore splinters the <b>top</b> face — which on this board is the face you marked, the face you can see, and on veneered ply the face whose thin outer ply tears in long strings. A reverse blade moves that tearout to the underside. Two things come with it: it <b>pushes the saw up off the work</b>, so hold the shoe down and feed slower than you would, and it wants a <b>flat, fully supported</b> cut — it is not the blade for a plunge. Also set the saw's <b>orbital selector</b> (the SELECTOR dial on this SKIL) to suit the stock: more orbit cuts faster and rougher, and on 3/4" ply the low settings are what keep the edge clean. That dial is the "set your jigsaw for the thickness of the wood" step, and it is the one most people never touch.</figcaption>
</figure>

<figure class="photo-figure">
<img src="photos/ply-05-comb-cut.jpg" alt="The finished one-piece anchor board comb standing against the garage wall, showing the full-width bridge across the top and three strips running down from it">
<figcaption><b>The anchor board, cut as ONE piece — bridge and all three strips continuous.</b> This is the part Section 8 spends the most words on, and here it is: no glue, no lap screws, no ply-to-ply joint anywhere in it. <b>The interior corners were drilled before they were sawn</b> — a bit dropped on the corner, then the jigsaw fed into the hole to finish the perpendicular cut. That is exactly the <b>~1/2" fillet at every inside corner</b> the plan calls for, arrived at from the practical end: the drilled hole is both the fillet and the jigsaw's entry, so the step that keeps the comb from starting a tear is the same step that lets you make the cut at all. Cut a square re-entrant corner and you get a stress riser <i>and</i> a blade you cannot turn. The two gaps came out as the offcuts the table above books for the battery drawer's walls — so <b>cut this board before that box</b> (Component 2).</figcaption>
</figure>

<figure class="photo-figure">
<img src="photos/ply-06-all-parts-cut.jpg" alt="Every cut plywood part for the build stacked and leaning against the garage wall, with the comb at right and ripped cleats alongside">
<figcaption><b>Both sheets, down to parts — this is what "two sheets, nothing else" looks like.</b> Panel C's deck, the kitchen box pieces, the battery drawer's walls and bottom, Panel C's front wall, the fridge tray, the ripped <b>3/4" × 3/4" bearer cleats</b> (the long thin strips standing at centre), and the comb at the right. Stand them on edge rather than stacking them flat: ply left lying in a pile in a garage picks up a bow from its own weight and from whatever humidity the slab is holding, and the deck and front wall are the two parts that have to stay flat. <b>Label each piece as it comes off the saw</b> — several of these are near-identical rectangles that differ by an inch, and the cut plan's names stop being obvious about a day later.</figcaption>
</figure>

<figure class="photo-figure">
<img src="photos/ply-07-wave3-shop-cooling.jpg" alt="The EcoFlow WAVE 3 running on a wire shelf in the garage, sitting on its add-on battery">
<figcaption><b>The WAVE 3 earned its keep before it was ever installed.</b> The same air conditioner this build carries (Component 8, and the cooling appendix) spent the cut days cooling the garage, stacked on its add-on battery exactly as it sits in the van. That is a free rehearsal worth paying attention to: it is where you find out how loud it is, how long the battery really lasts at your ambient, and how much clearance the rear intake grille actually needs behind it — all of which set the WAVE 3 bay's dimensions in Panel A, and none of which a spec sheet tells you.</figcaption>
</figure>
...
| Bottom rails (cube frames) | 5x 46" + 2x **23¼"** | see notes | underside at 1" (dropped to the leg bottoms): Panel A both END faces (2x46), Panel B all 4 faces (2x46 + 2x23), Panel C FRONT face only (1x46) — 2x 2½" screws + glue into each leg. **OFFSET those screws ~⅜" either side of the leg’s centreline (owner, Sept 2026):** the leveling-foot bore is ½" dia × 1¾" deep up the leg’s end grain, which is DEEPER than the rail band is tall, so a centred screw hits the insert at every height. ~½" of solid wood each side of the bore takes them. Applies to all 14 bottom-rail-to-leg joints. They screw into the legs, so they sit on the legs' line: 1.5" inboard wherever the legs are lapped, which is what took Panel B's side pair from 26⅛" to 23¼" |
...
| FOUND STORAGE — DELTA 3 drawer top tray (reclaims ~2.1" of dead headroom above the DELTA 3 stack — 13.75" box − 1/2" bottom − 11.16" stack; was ~3.3" before the deck recess: cables, the DELTA 3's own cords, dongles) — shallow lift-out bin, **build it 1.75" or shallower** | 1 | $8 | $8 |
...
- Corner brackets — 4
- Diagonal corner braces — 2 (recovers racking rigidity lost without a top). **They mount on the frame's UNDERSIDE**, not the rail tops — the top face is the deck plane and the bed section is screwed down flat onto it, so anything proud of it is a rock you can never shim out. Same face as the 4 corner brackets. Note the frame is built top-face-DOWN, so that face is the one pointing up at you on the bench
- Drawer box — 1 (right/DELTA 3 side only), 5 pieces: bottom 19.25"x21.25" (1/2" ply, into a 3/8" glued rabbet), 2 side walls 22"x13.75" + 2 front/back walls 18.5"x13.75" (cut at 20", trimmed 1.5" each) (**3/4" ply, Aug 2026** — the 3/8" purchase was deleted and these are the parts no other leftover could hold, Section 3). Glue + **2× R3** per corner since it carries the 48 lb DELTA stack. **Two of the four walls come out of the anchor board's comb gaps, so cut the anchor board (Section 8) first**
...
<p class="lego-caption">Build the right (DELTA 3) drawer box (E, 5 pieces, open top) — biscuit the corners with **2x R3** biscuits each + glue, bottom in a glued rabbet (**R3, not R1** — the walls went to ¾" in Aug 2026 and an R1 is thin-stock sizing, lost in that thickness; Joinery guide, vignette 3) (Joinery & Fastener Guide) — and slide it onto its pair of 20" full-extension slides (F) — one slide side on the frame's outer rail, the other on the center divider. Add a catch (G) so it doesn't slide open in transit. On the LEFT bay, skip the drawer entirely: glue/screw down the 2 glide strips (H) along the floor instead — that's the WAVE 3's storage spot (Component 8). <strong>The full shop drawing for this box — plan, section, corner detail, fit checks, cut list and assembly order — is the DELTA 3 Power Drawer sheet appended at the end of this PDF.</strong></p></div>
</div>
<div class="lego-step">
...
<figcaption><b>Clamp the leg before you drill it — and cut a block so you only measure once.</b> A <b>lapped</b> leg has to sit flush with the rail top and stay exactly there while the pilot and both SPAX screws go through the rail's face into the leg's side (Section 3). Unclamped, it walks under the bit, and it ends up proud of the rail — which is the one surface the bed platform lands on. Two bar clamps hold it; the offcut marked <b>2"</b> sets the leg's position across the panel. <b>Be exact about what that 2" spans, because it is not the leg inset:</b> it is the <b>gap between the side rail's inner face and the leg's outer face</b>. The side rail sits flush with the deck's edge, so 1&#8209;7/16" of measured rail plus the 2" block puts the leg's outer face <b>3&#8209;7/16"</b> in from the edge — the 3.5" <code>leg_inset</code> the model carries, to within 1/16", and still 3/16" clear of the 3&#188;" vent intrusion that set that number (V4). Read the 2" as an inset instead and you would move every leg 1&#189;" outboard, straight into the vent. <b>It is a gauge, not a shim:</b> cut it once, register the leg against it at every corner, and the position never gets measured again. Cheapest jig in the build.</figcaption>
</figure>
...
- Corner brackets — 4
- Diagonal corner braces — 2 (**underside**, like Panel A's — see Component 2)
...
<div class="lego-main">
<p class="lego-caption">Slide the WAVE 3 into Panel A's left bay, resting on the 2 glide strips — no box, no slide hardware, just reach in through the driver's side door. Mount the hose/cord hook in the open utility bay (screwed up into the deck underside at the bay's kitchen side; bundle the hoses with their own strap so nothing swings out of the open bay) for when the unit is carried off to run at camp. <strong>Found storage</strong> (DELTA 3/WAVE 3 detail render, side sections): the DELTA 3 stack is only 11.16" tall in a 13.75" box — 13.25" clear over the 1/2" bottom — so drop a shallow lift-out tray, **1.75" or less**, in the ~2.1" of dead air on top for cables/cords (the old 2" figure was sized against the deleted 3/8" bottom, and the cam strap runs under the tray); and the WAVE 3 is 13.2" in the 16.25" bay, so screw a thin shelf onto 1x1 cleats at ~13.7" — flat soft goods and the WAVE 3's own hoses ride up there while the unit still slides out beneath it. Neither touches the frame or the units' function.</p></div>
</div>
<div class="lego-step">
<div class="lego-num">3</div>
...
- **No wood joints at all — cut the whole comb as ONE piece (assembly sheet, V5).** The bridge and all three strips come out of a single 46" x 33" blank, so **no glue and no lap screws appear anywhere on this board**; every fastener left on it holds *hardware* to the ply, not ply to ply. It fits the stock you already buy: the 3/4" sheet carries only Panel C's deck plus some ripped cleats, leaving a contiguous spare of at least 48" x 60" — the comb drops in with ~26" of sheet length still left over for the control-panel backer board, the rear-pantry hold-down cleats and the spare-tire skid battens. Three things to get right when you cut it:
  ...
  - **Fillet every inside corner, ~1/2" radius** — drill a 1/2" hole centered *on* the corner first, then saw into it. A square re-entrant corner is where a comb starts a tear, and it is the only place this one-piece board is weaker than three separate strips would have been. **Cut Sept 2026, and the drill-first rule paid twice** — the hole is the fillet *and* the jigsaw's entry into the perpendicular cut, so it is not an extra step you can skip. Use a **reverse-tooth jigsaw blade** (Bosch T101BR or equivalent) so the tearout lands on the underside rather than the marked face: photos and the rest of the method in *The sheets, as actually cut* (Section 3).
...
1. Panel A frame: rails + 4 legs + divider + 2 END-face bottom rails (underside 1") + 2 diagonal braces (**on the frame's underside**, same face as the corner brackets). Corners: 2× 2½" screws + bracket + glue.
...
**1/2" Baltic birch plywood — 1 sheet (4×8):** kitchen-drawer box (bottom 1× **18×26"**; sides 2× 26×4"; front/back 2× **17×4"**) + hanging cheeks 2× **26×5.45"**; battery-drawer bottom 1× **19.25×21.25"**; **Panel C front wall 1× 46×17"**; **fridge tray 1× 17.72×28.74"** (from a 29×18 blank, cross-grain). **No 3/8" sheet** — deleted Aug 2026.

**Battery-drawer walls — 3/4", from the 3/4" sheet's offcuts:** 2× **22×13.75"** sides + 2× **18.5×13.75"** front/back. One of each comes out of the 47×19" strip; the other two come out of the anchor board's comb gaps, so **cut the comb first**. 2× R3 biscuits per corner.
...
- **The anchor board's comb outline** — a full-width 46 × 6" bridge with three strips 27" long running back from it (2.5" / 4.65" / 1.5" wide, at X = 0, 19.35, 44.5), ~½" fillet at each inside corner. Jigsaw/router from the 46 × 33 blank. **Its two gaps are parts now, not offcuts:** gap A (16.85 × 27") gives a **battery drawer side, 22 × 13.75**, gap B (20.5 × 27") gives a **battery drawer back, 18.5 × 13.75** — both rotated out of the gap, so mind the grain. **Cut the comb before you build the drawer box.**
- **The other two battery-drawer walls** — one **22 × 13.75** side and one **18.5 × 13.75** front, side by side out of the ¾" sheet's 47 × 19 strip (22 + 18.5 = 40.5 of the 47, with 6.5" to spare since the lapped legs shortened the drawer). Rip the strip to 13.75" wide first, then two crosscuts; the 47 × 5.25 band left over is pantry-cleat stock. All four walls take **2 × R3** biscuits per corner, not R1.
...
| S6 | Rip piece **C** to **19¼"** wide, then crosscut to **21¼"** | **BATTERY DRAWER BOTTOM, 19¼ × 21¼** · plus **27¾ × 25 → KEEP** (the lift-out tray comes out of this) and a 19¼ × 3¾ drop |
